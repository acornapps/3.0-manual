...
| 조치 사항 | Alertmanager의 로그와 이벤트를 확인한다. 필요할 경우, Pod를 재시작한다. |
...
| 지속 시간 | 10분 |
| 발생 조건 | 최근 5분 동안의 wal fsync 지속 시간의 99번째 백분위가 500ms보다 클 경우 \(wal fsync는 로그 항목을 적용하기 전에 디스크에 유지할 때 호출됨.\) |
| 조치 사항 |  |
...
| 지속 시간 | 10분 |
| 발생 조건 | 최근 5분 동안의 커밋 지속 시간 중 99번째 백분위가 250ms보다 클 경우 \(backend commit은 디스크에 대한 최근 변경 사항의 증분 스냅 샷을 커밋.\) |
| 조치 사항 |  |
...
| 조치 사항 | 해당 노드의 kubelet의 상태를 확인한다. |
...
| 지속 시간 | 1시간 |
| 발생 조건 | 클러스터 전체에서 3%이상의 kublet 메트릭 수집이 안 될 경우 발생 |
| 조치 사항 |  |
...
| 알람 이름 | K8SKubeletDown |
| 지속 시간 | 1시간 |
| 발생 조건 | 클러스터 전체에서 10%이상의 kublet 메트릭 수집이 안 될 경우 발생 |
| 조치 사항 |  |
